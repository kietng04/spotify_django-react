...
const EditArtist = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        alert(`Nghệ sĩ "${data.name}" đã được cập nhật thành công!`);
        navigate("/artists/manage");
      } else {
        const errorData = await response.json();
        setErrorMessage(errorData.detail || "Không thể cập nhật nghệ sĩ");
      }
    } catch (error) {
  ...
  if (isLoading) {
    return <div>Đang tải dữ liệu nghệ sĩ...</div>;
  }
        ...
        <div className="wrapper">
          <div className="content">
            <div className="content_item">
              <h2 className="sub_heading">Chỉnh sửa nghệ sĩ #{artistId}</h2>
            ...
            <div className="sidebar_item">
              ...
              <div className="column">
                ...
                <small style={{ display: "block", marginTop: "5px" }}>
                  Nhấp vào trên đây để chọn hình ảnh mới từ máy tính của bạn
                </small>
              </div>
            </div>
          </div>
