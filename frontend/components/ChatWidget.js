...
export default function ChatWidget() {
  ...
  const fetchConversations = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const formattedConversations = data
          .map((conv) => {
            ...
            return {
              ...
              timestamp: convTime.toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              }),
        ...
        const geminiConversation = formattedConversations.find(
          (conv) => conv.user_id === 1
        );
  ...
  const searchUsers = async (query) => {
    ...
    if (!token) {
      console.error(
        "Không có token, không thể tìm kiếm. Vui lòng đăng nhập lại."
      );
  ...
  const findConversationWithUser = async (userId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/conversations/search/?user_id=${userId}&_=${new Date().getTime()}`,
        {
          headers: {
            Authorization: `Token ${token}`,
      ...
      if (response.ok) {
        ...
        if (data && Array.isArray(data) && data.length > 0) {
          const filteredConversations = data.filter(
            (conv) => conv.other_user && conv.other_user.id === parseInt(userId)
          );
  ...
  const createConversation = async () => {
    ...
    try {
      console.log(
        `Tạo cuộc trò chuyện với người dùng: ${selectedUser.username} (ID: ${selectedUser.id})`
      );
      const userData = localStorage.getItem("spotify_user");
      const token = userData ? JSON.parse(userData).token : null;
      const response = await fetch(
  ...
  const ensureWebSocketConnection = () => {
    ...
    if (
      ...
      if (token && user?.user_id) {
        ...
        console.log(
          "Re-establishing WebSocket connection before sending message"
        );
  ...
  const sendMessage = async (conversationId, content) => {
    ...
    try {
      ...
      const isGeminiConversation =
        activeConversation?.user_id === 1 || conversationId === "gemini-ai";
      ...
      if (isGeminiConversation) {
        ...
        try {
          ...
          const tokenz = userDataString
            ? JSON.parse(userDataString).token
            : null;
      ...
      console.log(
        `Gửi tin nhắn đến cuộc trò chuyện ${conversationId}: "${content}"`
      );
      const activeSocket =
  ...
  const handleSelectUser = async (user) => {
    ...
    try {
      ...
      if (existingConversation && existingConversation.id) {
        console.log(
          "Đã tìm thấy cuộc trò chuyện hiện có:",
  ...
  const handleStartChat = async () => {
    ...
    try {
      const existingConversation = await findConversationWithUser(
        selectedUser.id
      );
      ...
      if (existingConversation) {
        ...
        console.log(
          "Chưa có cuộc trò chuyện, tạo mới với:",
  ...
  return (
    <Box position="fixed" bottom="74px" right="20px" zIndex="1000">
      {isOpen && (
        <Box
          // position="absolute"
          bottom="70px"
            ...
            <Flex direction="column" h="calc(100% - 57px)">
              ...
              >
                <Avatar
                  ...
                  src={
                    activeConversation.avatarImg
                      ? formatImageUrl(activeConversation.avatarImg)
                      : undefined
                  }
                  mr={2}
                />
              ...
              >
                {messages.map((msg, index) => (
                  <Flex
                    key={msg.id || index}
                    justifyContent={
                      msg.sender === user?.user_id ? "flex-end" : "flex-start"
                    }
                    width="100%"
                    mb={2}
                  >
                    <Box
                      ...
                      borderTopRightRadius={
                        msg.sender === user?.user_id ? 0 : "lg"
                      }
                      borderTopLeftRadius={
                        msg.sender === user?.user_id ? "lg" : 0
                      }
                    ...
                    >
                      ...
                      <Text
                        fontSize="xs"
                        color={
                          msg.sender === user?.user_id ? "white" : "gray.500"
                        }
                        textAlign="right"
                      >
              ...
              <Box mt={4} p={3} borderWidth="1px" borderRadius="md">
                ...
                <Flex alignItems="center" mb={3}>
                  <Avatar
                    ...
                    src={
                      selectedUser.avatarImg
                        ? formatImageUrl(selectedUser.avatarImg)
                        : undefined
                    }
                    mr={2}
                  />
